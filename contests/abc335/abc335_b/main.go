	...
	for x := 0; x < 22; x++ {
		for y := 0; y < 22; y++ {
			for z := 0; z < 22; z++ {
				...
				if _, ok := m[[3]int{x, y, z}]; !ok {
					m[[3]int{x, y, z}] = true
					zz = z
					print(iToS(x) + " " + iToS(y) + " " + iToS(z))
				}
			}
			if (x + y + zz) > n {
				break
			}
			if _, ok := m[[3]int{x, y, zz}]; !ok {
				m[[3]int{x, y, zz}] = true
				yy = y
				print(iToS(x) + " " + iToS(y) + " " + iToS(zz))
			}
		}
		if (x + yy + zz) > n {
			break
		}
		if _, ok := m[[3]int{x, yy, zz}]; !ok {
			m[[3]int{x, yy, zz}] = true
			print(iToS(x) + " " + iToS(yy) + " " + iToS(zz))
		}
	}
}

...
func main() {
	...
	for i := 0; i < n; i++ {
		a := int1()
		na = append(na, ab{abi: i, v: a, isA: true})
	}
	...
	for i := 0; i < m; i++ {
		a := int1()
		nb = append(nb, ab{abi: i, v: a, isA: false})
	}

	nc := append(na, nb...)

	sort.Slice(nc, func(i, j int) bool {
		return nc[i].v < nc[j].v
	})
	...
	sort.Slice(resa, func(i, j int) bool {
		return resa[i].abi < resa[j].abi
	})

	sort.Slice(resb, func(i, j int) bool {
		return resb[i].abi < resb[j].abi
	})

	aa := make([]int, len(resa))
	bb := make([]int, len(resb))
	for i, v := range resa {
		aa[i] = v.ci + 1
	}
	for i, v := range resb {

...
func main() {
	...
	bPlus := make([]int, 0, len(b))
	for _, v := range b {
		if v > 0 {
			bPlus = append(bPlus, v)
		} else {
			break
		}
	}

	wPlus := make([]int, 0, len(w))
	for _, v := range w {
		if v > 0 {
			wPlus = append(wPlus, v)
		} else {
			break
		}
	}
	if len(wPlus) > len(bPlus) {
		lenbp := len(bPlus)
		lenwp := len(wPlus)
		wPlus = wPlus[:len(bPlus)]
		for i := lenbp; i < lenwp; i++ {
			if len(b) > i && b[i]+w[i] > 0 {
				bPlus = append(bPlus, b[i])
				wPlus = append(wPlus, w[i])
			}
		}
	}
	max := 0
